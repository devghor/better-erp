...
- **Authentication**: Auth.js (NextAuth v5) with a Credentials provider
...
- Auth.js (NextAuth v5) with a Credentials provider for authentication
- JWT session strategy; session carries `id`/`role` (see `src/auth.ts`)
- In-memory mock user store by default (`src/constants/mock-api-auth.ts`) — swap for a real DB
- Client-side RBAC for navigation visibility, based on session `role`
...
│   ├── api/               # API routes (incl. api/auth/[...nextauth])
...
/docs                      # Documentation
│   ├── auth_setup.md      # Auth.js (Credentials) configuration guide
...
```env
# Generate with `npx auth secret` or `openssl rand -base64 33`. Optional in dev.
AUTH_SECRET=
```

Demo login: `demo@example.com` / `demo12345` (see `src/constants/mock-api-auth.ts`).
...
**Note**: In development, Auth.js works without `AUTH_SECRET` set (it warns and uses an ephemeral secret) — set it before deploying to production.
...
- `role: string` - Requires the session user to have a specific role

### Client-Side Filtering

The `useFilteredNavItems()` hook in `src/hooks/use-nav.ts` filters navigation client-side using Auth.js's `useSession()` hook (`session.user.role`). This is for UX only - actual security checks must happen server-side.
...
`src/proxy.ts` protects everything under `/dashboard(.*)` via the `authorized` callback in `src/auth.config.ts`, redirecting unauthenticated requests to `/auth/sign-in`. For server-side checks in a page or server action:
...
Check `session.user.role` server-side wherever role-gated logic is needed:
...
**Auth.js `AUTH_SECRET` warning**

- Normal in development when the env var is unset — Auth.js uses an ephemeral secret
...
- Verify the signed-in user has the required `role`
