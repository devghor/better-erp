...
- **Error handling**: All exception logging and JSON error rendering is centralized in `bootstrap/app.php`'s `->withExceptions()` — there are no scattered `Log::` calls or per-controller try/catch. `$exceptions->context()` attaches `user_id`/`url`/`method` to every logged fault automatically. API error responses use the standard envelope `{"success": false, "message": ..., "errors": ...}` built by `App\Support\ApiResponse::error()`; success responses are untouched. Multi-step writes (e.g. `Auth\LoginController::issueTokens()`/`refresh()`) are wrapped in `DB::transaction()` so partial failures roll back and rethrow into the centralized handler — follow this pattern for any new controller/service method that performs more than one write.
